	...
	public void editarUsuario(){
		...
		if(!usuario.getUsername().isEmpty()){
			...
			if(usuarioDAO.update(usuario) != 0){
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Ocurri� un error al modificar el Usuario, "
						+ "int�ntelo nuevamente!", null);
			}
		}else{
			msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Username no puede estar vac�o!", null);
		}
		FacesContext.getCurrentInstance().addMessage(null, msg);
	}
...
//					Producto prod = productoDAO.get(idProd);
					...
					if (existe) {
						...
						ccCliente.setDetalle("Venta Consignaci�n nro: " + ventaCon.getId());
	...
	public void procesoActualizaUltimoCosto() {
		...
		for (Producto producto : listaProducts) {
			...
			System.out.println("M�vil: " + producto.getNombre() + " Actualiz�: " + updateo + " con ultimo costo: " + ultimoCosto);
		}
	}
