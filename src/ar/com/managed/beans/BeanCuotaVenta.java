...
public class BeanCuotaVenta implements Serializable {
	...
	public String goCuota(Venta ven, Usuario user) {
		try{
			...
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "No es posible acceder a la vista de Cuotas, Int�ntelo nuevamente!", null);
			FacesContext.getCurrentInstance().addMessage(null, msg);
			return "";
		}		
	}
		...
		if (!selectedMoviles.isEmpty() && primerVencimiento != null && cantCuota != 0) {
			try {
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Ocurrio un Error al generar las cuotas, "
						+ "int�ntelo nuevamente mas tarde!, Error original: " + e.getMessage(), null);
			}
		} else {
	...
	public void pagarCuota(CuotasVentasDetalle cuotaVentaDet) {
		...
		if (idCaja != 0 && idCuentaCorriente != 0 && idCuotDet != 0) {
			...
			msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Ocurri� un error al registrar el pago de la cuota, "
	...
	public void bajaDePago(CuotasVentasDetalle cuotaVentaDetalle) {
		...
		try {
			...
			if (idCuotaDet != 0 && deleteCaja != 0 && deleteCuenta != 0) {
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Ocurri� un error al dar de baja el pago de la Cuota, "
						+ "int�ntelo nuevamente!", null);
			}
		} catch (Exception e) {
	...
	public void bajaDeCuota(CuotasVenta cuotaVen) {
		...
		try {
			...
			if (idCuota != 0) {
				...
				msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Ocurri� un error al dar de baja la Cuota, "
						+ "int�ntelo nuevamente!", null);
			}
			FacesContext.getCurrentInstance().addMessage(null, msg);
		} catch (Exception e) {
