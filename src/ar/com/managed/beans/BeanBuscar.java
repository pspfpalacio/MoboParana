...
public class BeanBuscar implements Serializable {
	...
	public void buscar(){
		...
		try{
			...
			if(unidadMovil.getId() != 0 && idProducto != 0){
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Debe colocar un nro de imei o seleccionar un producto, no puede realizar una b�squeda por ambos!", null);
				FacesContext.getCurrentInstance().addMessage(null, msg);
			}
			if(unidadMovil.getId() != 0 && idProducto == 0){
